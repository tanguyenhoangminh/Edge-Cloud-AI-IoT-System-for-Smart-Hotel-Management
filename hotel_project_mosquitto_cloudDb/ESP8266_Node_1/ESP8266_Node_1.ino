...
const char* TOPIC_SENSORS_2 = "hotel/room/" ROOM_1 "/sensors";  

 
const char* TOPIC_CTRL_2    = "hotel/room/" ROOM_1 "/control";  
...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  if (strcmp(topic, TOPIC_CTRL_2) == 0) {
    handleCommand(ROOM_1, (const char*)payload, length);
  }
}
...
void connectMQTT() {
  ...
  while (!mqtt.connected()) {
    ...
    if (mqtt.connect("ESP8266_Hotel_0101", MQTT_USER, MQTT_PASS)) {
...
void loop() {
  ...
  if (now - lastSensorMs >= SENSOR_INTERVAL_MS) {
    ...
    Serial.print("[PUB] SENSORS TO ROOM 0101 => "); 
    Serial.println(out);
  }
}