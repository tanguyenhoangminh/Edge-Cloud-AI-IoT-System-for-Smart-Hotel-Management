...
app.put('/api/iot/:room_number/control', async (req, res) => {
    ...
    try {
        ...
        if (deviceKey === 'door_lock') {
            await pool.query(`UPDATE room_iot_state SET door_lock = ?, door_open = ? WHERE room_id = ?`, [value, !value ? 1 : 0, roomId]);
        } else {
        ...
        if (brightness !== undefined && deviceKey === 'main_light') {
            await pool.query(`UPDATE room_iot_state SET light_brightness = ? WHERE room_id = ?`, [brightness, roomId]);
        }
